...
------------------
#### 追加機能
- 更新と変換の同時実行を実装しました。環境設定の concurrency オプションを有効に
  することで使用可能になります
  + とっても試験実装中（β版）。動作がおかしくなったら不具合報告を！
  + 表示されるログの出力形式が変更されます。CUI では特殊な形式になり、WEB UI
    ではコンソールが物理的に２つになります
- ログ保存機能を一新しました
  + 環境設定に logging オプションを追加しました。有効にすることでほぼすべての
    ログを保存します(diff や list コマンド、ヘルプは保存しません)
    * diff や list コマンド、ヘルプなどの、一部出力が多く表示すること自体が
      目的のコマンドの出力はログには保存しません
  ...
  + log コマンドを実装しました。logging オプションで保存されるログを表示する
    ...
    * concurrency で更新と変換を同時実行中は基本的に更新のログだけ表示されるの
      ...
      もう片方で narou log -ct で更新ログを垂れ流すみたいな使い方を想定

#### WEB UI
- コンソールに「過去ログを取得」ボタンを追加しました
  + 普段はリロードしたら直近のログ以外は表示されませんが(読み込み負荷軽減の為)
    それを再表示する機能です。過去ログはログ削除ボタンを押した場合に表示と共に
    一緒にクリアされます
  + ここで云っている「過去ログ」とは、サーバが起動してからWEB UIのコンソールに
    表示されたログのことで、logging オプションを有効にして保存されたログとは
    関係ありません
