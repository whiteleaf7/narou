...
- コマンド使用時に `^tag:NAME` でタグの除外指定が出来るようになりました。
  ...
  -tag とするとオプションと誤認識されてしまう問題があり、^ に変更しました。
	+ これに合わせて、WEB UI でも ^tag: を使えるようになります（従来の -tag も
	  使えます）
- WEB UI の Update ボタンに「タグを指定して更新」を追加しました
- WEB UI の Update ボタンの「最新話更新日を確認」を刷新しました。なろうAPIを
  使用して一括で更新されたかを確認することた出来るようになります
	+ update --gl オプションも同様の機能に変わっています。詳細は narou u -h を
	  参照
- WEB UI の小説リストに「話数」を追加しました
- Linux でも WEB UI の Eject ボタンに対応しました
...
- 「更新はありません」と出た場合も更新日を変更するようにしました
