...
- ダウンロード（や更新）と変換の同時実行が可能になりました
  ...
  + 試験実装なので、仕様変更の可能性有り。使う場合はバックアップを万全に
...
- 環境設定に filename-length-limit を追加しました。ファイルを保存する際の長さ
  制限を設定出来ます。（デフォルトでは50文字）
  + 既存作品のアップデートにも影響が出るので、50文字でも長すぎてエラーが出る
    場合のみ変更を推奨
...
- タイムアウトなどのネットワークエラーや、手動でダウンロードを中断した場合でも
  データベースへの登録は行うように変更しました。update コマンドで中断したところ
  からダウンロードを再開出来るようになります
- 中断した場合でも、そこまでの新着に関しては hotentry を生成するようになります
