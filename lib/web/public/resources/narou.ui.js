...
$(document).ready(function() {
    ...
    columns: [
        ...
        searchable: false, visible: cell_visible.download },
      {
        ...
        createdCell: function(td, cellData, rowData, rowIndex, colIndex) {
          $(td).children("button")
            .data("targetId", rowData._id)
            .on("click", function(e) {
              e.stopPropagation();
              var $this = $(this);
              var $tr = $(e.target).closest("tr").addClass("context-menu-target");
              var target_id = $this.data("targetId");
              context_menu.open(target_id, get_event_position(e), function() {
                $tr.removeClass("context-menu-target");
              });
            });
        },
  ...
  var context_menu = new Narou.ContextMenu(action);

  notification.on("table.reload", function() {
    table.ajax.reload(function() {
      $("#badge-number-of-selecting").text(0);
    });
  });
  ...
  /*
   * ボタン
   *   Update > 選択した小説をアップデート
   */
  $("#button-update-select").on("click", function(e) {
    e.preventDefault();
    action.updateSelect();
  });
   ...
   *   Send > 選択した小説を送信
   */
  $("#button-send-select").on("click", function(e) {
    e.preventDefault();
    action.sendSelect();
  });
