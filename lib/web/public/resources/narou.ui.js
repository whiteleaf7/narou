...
$(document).ready(function() {
  ...
  var table = t = $("#novel-list").DataTable({
    ...
    initComplete: function(_settings, _json) {
      ...
      table.$("a")
        ...
        .attr("ondragstart", "return false");  // Firefox用
    },
    language: {
  ...
  function disable_menu_item(id) {
    $(id).prop("disabled", true).on(click_event_name, nothing_to_do);
  }

  function enable_menu_item(id) {
    $(id).prop("disabled", false).off(click_event_name, nothing_to_do);
  }
  ...
  var top_dnd_function = function(e) {
    $("#link-drop-here").show();
    Narou.noScroll();
  };
  window.onTopDragEnter = function() {
    $(window).on("dragenter", top_dnd_function);
  }
  window.offTopDragEnter = function() {
    $(window).off("dragenter", top_dnd_function);
  }
