...
$(document).ready(function() {
  ...
  var table = t = $("#novel-list").DataTable({
    ...
    columns: [
      ...
      { title: "更新日",
        ...
        render: function(data, type, row) {
          if (type === "display" || type === "filter") {
            ...
            var display = Narou.formatDate(date, "YYYY/MM/DD hh:mm");
            if (type === "display") {
              var klass = null;
              if (row.new_arrivals_date) {
                if ((row.new_arrivals_date + NEW_ARRIVALS_LIMIT) * 1000 >= now) {
                  //display += "&nbsp;<span class=new-arrivals>新着</span>";
                  //display = "<span class=new-arrivals>" + display + "</span>";
                  klass = "new-arrivals";
                }
              }
      ...
      // 新着時間（内部データ用）
      { title: "新着時間", data: "new_arrivals_date", visible: false },
    ].map(function(hash) {
      // 検索用に name 要素を追加
      hash.name = hash.data;
    ...
    createdRow: function(row, data, data_index) {
      ...
      if (isToday(data.last_update)) {
        //$(row).addClass("today-update");
      }
    },
  ...
  notification.on("table.reload", function() {
    // リロードする前に現在の選択状態を保存
    var selected_ids = $.map(table.rows(".selected").data(), function(e) {
      return e.id;
    });
    table.ajax.reload(function() {
      // リロードしたら選択状態は解除されているので復元
