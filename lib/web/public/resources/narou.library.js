...
var Narou = (function() {
  ...
  $.extend(ContextMenu.prototype, {
    _default_symbols: [
      "setting", "diff", "edit-tag", "freeze-toggle", "update",
      "send", "remove", "convert", "inspect", "folder", "backup"
    ],

    items: [
      { label: "――――――――(区切り)", symbol: "divider" },
      { label: "小説の変換設定", symbol: "setting" },
      { label: "差分を表示", symbol: "diff" },
      { label: "タグを編集", symbol: "edit-tag" },
      { label: "凍結 or 解凍", symbol: "freeze-toggle" },
      { label: "更新", symbol: "update" },
      { label: "送信", symbol: "send" },
      { label: "削除", symbol: "remove" },
      { label: "変換", symbol: "convert" },
      { label: "調査状況ログを表示", symbol: "inspect" },
      { label: "保存フォルダを開く", symbol: "folder" },
      { label: "バックアップを作成", symbol: "backup" },
    ],
    ...
    initializeMenuObject: function() {
      var menu_text = storage.get("context_menu_text") || this.createDefaultMenuText();
      this.object = this.createMenuObject(menu_text);
    ...
    initializeMenuEvent: function() {
      ...
      var self = this;
      object.find(".context-menu-setting").on("click", function(e) {
        e.preventDefault();
      ...
      object.find(".context-menu-update").on("click", function(e) {
        e.preventDefault();
        self.openConsoleDialog(function() {
          self.action.updateForce(self.target_id);
        });
      });
      object.find(".context-menu-send").on("click", function(e) {
        e.preventDefault();
        self.openConsoleDialog(function() {
          self.action.send(self.target_id);
        });
      });
      object.find(".context-menu-freeze-toggle").on("click", function(e) {
        e.preventDefault();
        self.action.freeze(self.target_id);
      });
      object.find(".context-menu-remove").on("click", function(e) {
        e.preventDefault();
        self.openConsoleDialog(function() {
          self.action.remove(self.target_id);
        });
      });
      object.find(".context-menu-edit-tag").on("click", function(e) {
        e.preventDefault();
        self.tag.openEditor(self.target_id);
      });
      object.find(".context-menu-convert").on("click", function(e) {
        e.preventDefault();
        self.openConsoleDialog(function() {
          self.action.convert(self.target_id);
        });
      });
      object.find(".context-menu-diff").on("click", function(e) {
        e.preventDefault();
        self.openSelectDiffListDialog(self.target_id);
      });
      object.find(".context-menu-inspect").on("click", function(e) {
        e.preventDefault();
        self.openConsoleDialog(function() {
          self.action.inspect(self.target_id);
        });
      });
      object.find(".context-menu-folder").on("click", function(e) {
        e.preventDefault();
        self.action.folder(self.target_id);
      });
      object.find(".context-menu-backup").on("click", function(e) {
        e.preventDefault();
        self.openConsoleDialog(function() {
    ...
    createDefaultMenuText: function() {
      var self = this;
      var menu_text_lines = [];
      _.each(this._default_symbols, function(symbol) {
        var item = self.findItem(symbol);
        if (!item)
          $.error("invalid symbol(" + symbol + ")");
        menu_text_lines.push(item.label + "<>" + symbol);
      });
