...
var Narou = (function() {
  ...
  $.extend(Action.prototype, {
    ...
    download: function() {
      var urls = this._getSelectedIds(arguments);
      if (urls.length == 0) {
        var download_modal = bootbox.dialog({
          ...
          message: '<textarea id="download-input" class="form-control" rows="3"></textarea>' +
                   '<small style="color:#aaa">（Downloadを開かなくても画面内どこでもリンクをドラッグ＆ドロップすればDL出来ます）</small>',
          backdrop: true,
          buttons: {
            cancel: {
              label: "キャンセル",
    ...
    csvImport: function() {
      $.get("/api/csv/view/import", function(html) {
        window.offTopDragEnter();   // トップ画面へのD&Dを停止する
        ...
        box.on("hidden.bs.modal", function() {
          window.onTopDragEnter();   // モーダル閉じたらトップ画面へのD&Dを再開
        });
      });
    },
  });
  ...
  $.extend(DragDrop.prototype, {
    getLinkText: function(event, callback) {
      ...
      var link = data_transfer.getData("text/uri-list");
      if (link.length > 0) {
        callback(link);
      }
      ...
      $.each(data_transfer.files, function(i, file) {
        ...
        reader.onload = function(e) {
          link = self.parseLink(file.name, reader.result);
