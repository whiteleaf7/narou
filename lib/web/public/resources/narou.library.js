  ...
  $.extend(Search.prototype, {
    ...
    initializeEvents: function() {
      ...
      $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        if (_.isEmpty(self.filter_tags)) return true;
    ...
    appendTagToFilter: function(tag_name) {
      if (_.includes(this.filter_tags, tag_name)) {
        return;
      }
      else if (tag_name) {
        this.myfilter.val((this.myfilter.val() + " tag:" + tag_name).trim());
      }
      else {
    ...
    removeAllTagsByFilter: function() {
      var normal_words = this.selectNormalWords(this.myfilter.val().trim());
      this.myfilter.val(normal_words.join(" "));
    },

    selectNormalWords: function(string) {
      var words = string.split(/\s+/);
      var normal_words =
        _.select(words, function(word) {
          if (word.match(/^tag:(.+)$/i)) {
            this.filter_tags.push(RegExp.$1);
            return false;
          }
          else {
            return true;
    ...
    search: function(sync) {
      var fn = function() {
        this.filter_tags.length = 0;   // 配列クリア
        var filter_string = this.myfilter.val().trim();
        var normal_words = this.selectNormalWords(filter_string);

        // タグ以外の単語で通常検索し、タグ部分はフックAPIでフィルターする
        this.table.search(normal_words.join(" ")).draw();
  ...
  $.extend(Tag.prototype, {
    ...
    registerEvents: function($target, stop_bubbling) {
      ...
      $target.on("click", ".tag", args, function(e) {
        ...
        self.search.appendTagToFilter(tag_name);
