...
var Narou = (function() {
  ...
  $.extend(ContextMenu.prototype, {
    ...
    initializeMenuEvent: function() {
      ...
      $("#context-menu-update").on("click", function(e) {
        e.preventDefault();
        self.action.updateSelect(self.target_id);
      });
      $("#context-menu-send").on("click", function(e) {
        e.preventDefault();
        self.action.sendSelect(self.target_id);
      });
  ...
  $.extend(Action.prototype, {
    ...
    _removeConfirmDialog: function(title, callback) {
      var message = "";
      $.each(this.table.rows(".selected").data(), function(i, val) {
        message += val.title;
      });
    ...
    remove: function() {
      var ids = this._getSelectedIds(arguments);
      if (ids.length === 0) return;
      this._removeConfirmDialog("選択した小説を削除しますか？", function(result) {
        if (!result) return;
    ...
    removeWithFile: function() {
      var ids = this._getSelectedIds(arguments);
      if (ids.length === 0) return;
      this._removeConfirmDialog("選択した小説を“完全に”削除しますか？", function(result) {
        if (!result) return;
