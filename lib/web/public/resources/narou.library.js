...
var Narou = (function() {
  ...
  $.extend(Action.prototype, {
    ...
    updateGeneralLastup: function() {
      bootbox.dialog({
        title: "最新話掲載日の更新",
        message: '<p>凍結済みを除く各小説の最新話掲載日のみを更新して反映させます。<br>' +
        'なろうAPIを使える小説はAPIを使用して一括で更新するので、高速に掲載日を調べることが出来ます。</p>' +
        '<ul><li>検索除外されている小説はAPIでは取得出来ません。</li>' +
        '<li>最新話掲載日は通常のUpdateでも更新されます。</li>' +
        '<li>“実際に小説のデータが修正されてからなろうAPIに反映されるまで平均5分程度(最大2時間)の誤差があります。”（<a href="http://dev.syosetu.com/man/api/" target="_blank">公式サイト</a>から引用）</li></ul>' +
        '<p class="text-right"><label><input type="checkbox" id="update-gl-narou" checked> なろう小説をAPIで更新</label>' +
        '&nbsp;&nbsp;&nbsp;<label><input type="checkbox" id="update-gl-other"> その他の小説を更新</label>' +
        '</p>',
        backdrop: true,
        buttons: {
          cancel: {
          ...
          main: {
            ...
            callback: function() {
              ...
              $.post("/api/update_general_lastup", { option: option });
            }
          },
        }
      });
