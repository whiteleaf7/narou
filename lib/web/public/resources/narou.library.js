  ...
  $.extend(Action.prototype, {
    ...
    updateGeneralLastup: function() {
      bootbox.dialog({
        ...
        'なろうAPIを使える小説はAPIを使用して一括で更新するので、高速に掲載日を調べることが出来ます。その他の小説はひとつずつ確認するので時間がかかります。</p>' +
        '<ul><li>検索除外されている小説はAPIでは更新出来ません。</li>' +
        '<li>最新話掲載日は通常のUpdateでも更新されています。</li>' +
        '<li>更新が確認された小説には<code>modified</code>タグが自動で追加されます。Updateすることで削除されます。</li>' +
