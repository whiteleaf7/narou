...
class WebSocket
    ...
    if params[:server] # server
      ...
      unless @server.accepted_origin?(self.origin)
        raise(WebSocket::Error,
              format(("Unaccepted origin: %s (server.accepted_domains = %p)\n\n" +
      ...
      line = gets.chomp
      raise(WebSocket::Error, "bad response: #{line}") unless line =~ %r!\AHTTP/1.1 101 !n
      read_header
      ...
      if reply_digest != expected_digest
        raise(WebSocket::Error,
              format("security digest doesn't match: %p != %p", reply_digest, expected_digest))
      end
      @handshaked = true
  ...
  def receive
    ...
    when "hixie-75", "hixie-76"
      ...
      loop do
        ...
        if WebSocket.debug
          $stderr.printf(format("recv_frame> opcode:%d fin:%d payload:%p\n", opcode, fin ? 1 : 0, payload))
        end
        case opcode
        when OPCODE_TEXT
  ...
  def origin
    ...
    @header[name] || raise(WebSocket::Error, "%s header is missing" % name)
  end
  ...
  def close(code = 1005, reason = "", origin = :self)
    unless @closing_started
      ...
      when "hixie-75", "hixie-76"
        ...
        payload = if code == 1005
                    ""
                  else
                    [code].pack("n") + force_encoding(reason.dup, "ASCII-8BIT")
                  end
  ...
  def send_frame(opcode, payload, mask)
    if WebSocket.debug
      $stderr.printf(format("send_frame> opcode:%d masked:%d payload:%p\n", opcode, mask ? 1 : 0, payload))
    end
...
class WebSocketServer
  ...
  def initialize(params_or_uri, params = nil)
    ...
    @tcp_server = if params[:host]
                    TCPServer.open(params[:host], @port)
                  else
                    TCPServer.open(@port)
                  end
  end
...
if __FILE__ == $0
  ...
  if ARGV[0] == "server" && ARGV.size == 3
    ...
    warn("Usage:")
    warn("  ruby web_socket.rb server ACCEPTED_DOMAIN PORT")
    warn("  ruby web_socket.rb client ws://HOST:PORT/")
    exit(1)
