...
class SectionStripper
  def load_section(section)
    endoff = if section + 1 == @num_sections
               @data_file.length
             else
  ...
  def initialize(datain, verbose = true)
    ...
    puts format("Found SRCS section number %d, and count %d", srcs_secnum, srcs_cnt) if @verbose
    ...
    puts format("   beginning at offset %0x and ending at offset %0x", srcs_offset, srcs_length) if @verbose
